...
CREATE TABLE `auth_item` (
  ...
  `created_time` int(11) unsigned DEFAULT NULL,
  `updated_time` int(11) unsigned DEFAULT NULL,
  PRIMARY KEY (`name`),
...
CREATE TABLE `dept` (
  ...
  CONSTRAINT `fk_dept_dept_1` FOREIGN KEY (`dept_id`) REFERENCES `user` (`dept_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT;
...
CREATE TABLE `status` (
  ...
  CONSTRAINT `fk_status_status_1` FOREIGN KEY (`status_id`) REFERENCES `user` (`status_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT;
...
CREATE TABLE `title` (
  ...
  CONSTRAINT `fk_title_title_1` FOREIGN KEY (`title_id`) REFERENCES `user` (`title_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT;
...
CREATE TABLE `user` (
  `user_id` smallint(1) unsigned NOT NULL AUTO_INCREMENT COMMENT '职员编号',
  `user_name` varchar(5) COLLATE utf8_unicode_ci NOT NULL DEFAULT '' COMMENT '职员姓名',
  `password_hash` varchar(100) COLLATE utf8_unicode_ci NOT NULL DEFAULT '' COMMENT '职员密码',
